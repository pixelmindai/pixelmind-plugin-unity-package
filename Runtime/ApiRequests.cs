...
namespace PixelmindImaginarium
{
    public class ApiRequests
    {
        public static async Task<List<Generator>> GetGenerators(string apiKey)
        {
            ...
            {
                Debug.Log(getGeneratorsRequest.downloadHandler.text);
        ...
        public static async Task<int> CreateImagine(List<GeneratorField> generatorFields, string generator, string apiKey)
        {
            ...
            Debug.Log(parametersJsonString); // Outputs: {"Key1":1,"Key2":2}
            ...
            {
                var result =
                    JsonConvert.DeserializeObject<CreateImagineResult>(createImagineRequest.downloadHandler.text);

                Debug.Log(result.request.id);
                createImagineRequest.Dispose();
        ...
        public static async Task<Dictionary<string, string>> GetImagine(int imagineId, string apiKey)
        {
            ...
            else
            {
                ...
                {
                    Debug.Log(status.request?.file_url);
